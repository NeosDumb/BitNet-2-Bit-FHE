...
# Define missing enum values locally if needed
# 40 is LLAMA_FTYPE_MOSTLY_I2_S in llama.h
LlamaFileType_MOSTLY_I2_S = getattr(LlamaFileType, 'MOSTLY_I2_S', 40)
...
GGML_FILE_TYPE_TO_DATA_TYPE: dict[int, DataType] = {
    LlamaFileType.ALL_F32         : DT_F32,
    LlamaFileType.MOSTLY_F16      : DT_F16,
    LlamaFileType_MOSTLY_I2_S     : DT_I2,
    LlamaFileType.MOSTLY_Q8_0     : DT_Q8_0,
}


def get_data_type_for_tensor(ftype: int, name: str, tensor: LazyTensor) -> DataType:
...
class OutputFile:
    ...
    def write_tensor_data(self, ftype: int, model: LazyModel, concurrency: int) -> None:
        ...
        # elif ftype == LlamaFileType_MOSTLY_I2_S:
        #     # ndarrays = bounded_parallel_map(
    ...
    @staticmethod
    def write_all(
        fname_out: Path, ftype: int, params: Params, model: LazyModel, vocab: BaseVocab, svocab: gguf.SpecialVocab,
...
def pick_output_type(model: LazyModel, output_type_str: str | None) -> int:
    ...
    if output_type_str == "i2":
        return LlamaFileType_MOSTLY_I2_S
...
def convert_to_output_type(model: LazyModel, output_type: int) -> LazyModel:
    # for (name, tensor) in model.items():
    #     print(name)
    #     print(tensor)
    #     print(get_data_type_for_tensor(output_type, name, tensor))
    #     print(tensor.astype(get_data_type_for_tensor(output_type, name, tensor)))
    return {name: tensor.astype(get_data_type_for_tensor(output_type, name, tensor))
            for (name, tensor) in model.items()}
...
def default_outfile(model_paths: list[Path], file_type: int) -> Path:
    namestr = {
        LlamaFileType.ALL_F32:    "f32",
        LlamaFileType.MOSTLY_F16: "f16",
        LlamaFileType.MOSTLY_Q8_0:"q8_0",
        LlamaFileType_MOSTLY_I2_S:  "i2",
    }[file_type]
