...
size_t quantize_i2_s(const float * src, void * dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    ...
    for (int i = 0; i < n / QK_I2_S; i++) {
        for (int j = 0; j < QK_I2_S; j++) {
            ...
            if (fabs(val) < 1e-6) {
                ...
                // Mathematical optimization: sign invariance.
                // Since the scale (max) is non-negative, `val * scale > 0` mathematically reduces to `val > 0`.
                // This allows O(1) memory mapping by calculating max and bit-packing concurrently.
                q = val > 0 ? 2 : 0;
            }

            int group_idx = j / 32;
            int group_pos = j % 32;
            i2_weight[i * 32 + group_pos] |= (q << (6 - 2 * group_idx));
        }
    }

    // Capture remaining max if n is not perfectly divisible by QK_I2_S
    for (int k = (n / QK_I2_S) * QK_I2_S; k < n; ++k) {
        max = fmax(max, (double)fabs((double)src[k]));
    }
    ...
    for (int i = 0; i < n / QK_I2_S; i++) {
        for (int j = 0; j < QK_I2_S; j++) {
            ...
            int group_idx = j / 16;
            int group_pos = j % 16;
            i2_weight[i * 16 + group_pos] |= (q << (6 - 2 * group_idx));
        }
    }
