...
size_t quantize_i2_s(const float * src, void * dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    ...
    // Since max scale >= 0, the check `val * scale > 0` simplifies to `val > 0`.
    // This removes temporal dependency on global max and enables fusing max-scan
    // and quantization, eliminating O(N) heap allocations.
    double max = 0;
    ...
    // Remainder max scan if n % QK_I2_S != 0
    for (int i = (n / QK_I2_S) * QK_I2_S; i < n; ++i) {
