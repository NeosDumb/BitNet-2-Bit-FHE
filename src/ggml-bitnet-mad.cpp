...
size_t quantize_i2_s(const float * src, void * dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    ...
    // Mathematical Insight: Since global scale >= 0, the quantization condition
    // `val * scale > 0` simplifies to `val > 0`. This sign invariance enables us
    // to fuse max-scan, quantization, and bit-packing into a single O(n) pass,
    // eliminating intermediate heap allocations and providing O(1) memory mapping.
    double max = 0;
    ...
    // Mathematical Insight: Sign invariance (scale >= 0) allows fusion of
    // max-scan and quantization loops, granting O(1) memory mapping overhead.
