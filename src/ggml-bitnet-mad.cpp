...
size_t quantize_i2_s(const float * src, void * dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    ...
    for (int i = 0; i < n / QK_I2_S; i++) {
        for (int j = 0; j < QK_I2_S; j++) {
            ...
            // Fused quantize & pack (Mathematical Property: Sign Invariance)
            // val * scale > 0 simplifies to val > 0 since scale is strictly non-negative!
            uint8_t q8_val = 1;
            if (abs_val >= 1e-6) {
                q8_val = val > 0 ? 2 : 0;
            }
    ...
    for (int64_t rg = 0; rg < nrow4; rg++) {
        ...
        for (int64_t col = 0; col < n_per_row; col++) {
            ...
            // Fused max scan
            if (fabs((double)v0) > max) max = fabs((double)v0);
            if (fabs((double)v1) > max) max = fabs((double)v1);
            if (fabs((double)v2) > max) max = fabs((double)v2);
            if (fabs((double)v3) > max) max = fabs((double)v3);
