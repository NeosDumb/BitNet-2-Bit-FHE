...
size_t quantize_i2_s(const float * src, void * dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    ...
    // Mathematical Insight: Sign Invariance for O(1) Memory Mapping
    // Since `max` represents the absolute maximum scale and is non-negative,
    // the evaluation `val * max > 0` mathematically simplifies to `val > 0`.
    // This temporal decoupling eliminates the need to calculate the global scale
    // before bit-packing, allowing us to fuse loop iterations and eliminate O(N) heap allocations.
    double max = 0;
    ...
    for (int i = 0; i < n / QK_I2_S; i++) {
        for (int j = 0; j < QK_I2_S; j++) {
            float val = src[i * QK_I2_S + j];
            max = fmax(max, (double)fabs((double)val));

            uint8_t q8 = fabs((double)val) < 1e-6 ? 1 : (val > 0 ? 2 : 0);

            int group_idx = j / 32;
            int group_pos = j % 32;
            uint8_t temp = (q8 << (6 - 2 * group_idx));
    ...
    // Mathematical Insight: Sign Invariance for O(1) Memory Mapping
    // The evaluation `val * max > 0` mathematically simplifies to `val > 0`.
    // Loop fusion eliminates the O(N) heap allocations and merges passes.
    double max = 0;
    ...
    for (int64_t rg = 0; rg < nrow4; rg++) {
        ...
        for (int64_t col = 0; col < n_per_row; col++) {
            ...
            max = fmax(max, (double)fabs((double)v0));
            max = fmax(max, (double)fabs((double)v1));
            max = fmax(max, (double)fabs((double)v2));
            max = fmax(max, (double)fabs((double)v3));

            uint8_t q0 = fabs((double)v0) < 1e-6 ? 1 : (v0 > 0 ? 2 : 0);
            uint8_t q1 = fabs((double)v1) < 1e-6 ? 1 : (v1 > 0 ? 2 : 0);
            uint8_t q2 = fabs((double)v2) < 1e-6 ? 1 : (v2 > 0 ? 2 : 0);
            uint8_t q3 = fabs((double)v3) < 1e-6 ? 1 : (v3 > 0 ? 2 : 0);
