...
size_t quantize_i2_s(const float * src, void * dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    ...
    // Fused loop optimization: calculates max, quantizes, and packs in a single pass.
    // Mathematical insight: Instead of calculating max in one O(N) pass, allocating O(N) intermediate
    // memory, quantifying in a second O(N) pass, and finally packing in a third O(N) pass,
    // we fuse the max-tracking and packing together. The resulting physical memory constraint
    // changes from O(N) intermediate allocation to O(1), and execution becomes cache-friendly.
    double max = 0.0;
    ...
    // Fused loop optimization for 1x4 processing
    double max = 0.0;
    ...
    for (int64_t rg = 0; rg < nrow4; rg++) {
        ...
        for (int64_t col = 0; col < n_per_row; col++) {
            ...
            out[base + col] = (uint8_t)((q0 << 6) | (q1 << 4) | (q2 << 2) | (q3 << 0));
        }
    }
