...
size_t quantize_i2_s(const float * src, void * dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    ...
    // Type Conservation & SIMD Friction Reduction:
    // Mixing `double` and `float` introduces implicit conversions that cause unnecessary computational friction
    // and can stall the execution pipeline. By ensuring strict type conservation (using `float`, `fabsf`, and `fmaxf`),
    // the system maintains "momentum" by executing in native single-precision, maximizing computational efficiency.
    float max = 0.0f;
    ...
    // Type Conservation & SIMD Friction Reduction:
    // Utilizing native `float` instructions (`fabsf`, `fmaxf`) eliminates implicit type promotion to `double`,
    // avoiding the computational penalty and preserving the efficiency of the numerical evaluation pipeline.
    float max = 0.0f;
