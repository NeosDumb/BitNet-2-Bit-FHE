...
size_t quantize_i2_s(const float * src, void * dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    ...
    // Math insight: Sign invariance under scale > 0
    // if max > 0, val * max > 0 is mathematically identical to val > 0.
    // We can fuse max-scan, quantization, and bit-packing to avoid O(n) allocation
