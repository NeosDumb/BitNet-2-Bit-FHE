...
size_t quantize_i2_s(const float * src, void * dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    ...
    // Mathematical Optimization: Scale Invariance
    // Since `scale = max(|src|)` >= 0, the check `val * scale > 0` mathematically simplifies to `val > 0`.
    // This removes the temporal dependency on `scale`, allowing max-scan and quantization
    // to be fused into a single O(n) pass, eliminating the intermediate O(n) `q8` heap allocation.
    ...
    for (int i = 0; i < n / QK_I2_S; i++) {
        for (int j = 0; j < QK_I2_S; j++) {
            ...
            int group_idx = j / 32;
            int group_pos = j % 32;
            uint8_t temp = (q8_val << (6 - 2 * group_idx));
    ...
    // Mathematical Optimization: Scale Invariance
    // Fused max calculation and quantization, eliminating O(n) heap allocation.
