...
size_t quantize_i2_s(const float * src, void * dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    ...
    // Precision Domain Matching:
    // Mixing precision types (float and double) forces the compiler to insert conversion instructions,
    // which halves SIMD processing density and introduces physical latency. By operating strictly
    // in the single-precision domain (using fmaxf, fabsf, 1e-6f) matching the input type, we maximize
    // pipeline utilization and conserve computational energy without losing mathematical validity.
